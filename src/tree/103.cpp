...
vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
	...
	// This variable is used to indicate the parity of the layer number.
	int direction = 0;
	...
	while (!q.empty()) {
		...
		if (compared_tmp >= layerNumber) {
			...
			// if the layer is an odd layer, reverse the layerVector
			if (direction%2 == 1) {
				reverse(layerVector.begin(), layerVector.end());
			}
