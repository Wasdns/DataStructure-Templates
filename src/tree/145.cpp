...
vector<int> postorderTraversal(TreeNode* root) {
	...
	if (root == NULL) {
		...
		// get the postorder of left tree
		vector<int> l_ans = postorderTraversal(root->left);
		int l_ans_size = l_ans.size();

		// get the postorder of right tree
		vector<int> r_ans = postorderTraversal(root->right);
		...
		for (int i = 0; i < l_ans_size; i++) {
			ans[i] = l_ans[i];
		}
		for (int i = l_ans_size; i < ans_size; i++) {
			ans[i] = r_ans[i-l_ans_size];
		}
